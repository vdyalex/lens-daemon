...
- **`src/bridges/core_graphics`** -- Objective-C wrappers for CoreGraphics screen capture, display queries (`captureScreenRect`, `getMainDisplayWidth`, `getMainDisplayHeight`), and window metadata (`capturedWindowID`, `capturedWindowRectByID` via `CGWindowListCopyWindowInfo` — pure metadata, no screen-capture indicator)
...
- **`tracker_window_changes.go`** -- window move/resize evasion: polls the captured window's bounding rect by `CGWindowID` via `CGWindowListCopyWindowInfo`
