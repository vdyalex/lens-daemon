...
- **CLI interface**: Provide Cobra-based CLI with subcommands for daemon control: `daemon` (run pipeline with IPC), `start` (daemonize), `stop` (shutdown), `status` (check status), `logs` (stream logs), `restart` (stop + start), `set` (runtime settings).
...
- **Config flags**: Accept command-line flags on `daemon`, `start`, and `restart` commands (--api-key, --bot-token, --model, --system-prompt, --max-tokens, --log-level, --store-path, --output-method) and forward them as environment variables to child processes.
...
- **Set command**: Implement `set` IPC command that updates runtime settings (e.g., `output-method`). Used by `lensd set output-method <telegram|teleprompter>`.
...
- **Teleprompter grid positioning**: While the bounds hotkey is held, arrow keys (Up/Down/Left/Right) move the teleprompter on a percentage-based grid with configurable step (`TELEPROMPTER_GRID_STEP`, default 1%) and initial position (`TELEPROMPTER_GRID_INITIAL_COL`/`TELEPROMPTER_GRID_INITIAL_ROW`, default 0.5). Position wraps circularly. Rapid presses debounce with a configurable delay (`TELEPROMPTER_GRID_MOVE_DEBOUNCE_DURATION`, default 300ms).
...
- **Output method**: Route the AI response to either the teleprompter overlay or Telegram subscribers based on `OUTPUT_METHOD` (default: `teleprompter`). Switchable at runtime via `lensd set output-method <telegram|teleprompter>`. When switching to telegram, the teleprompter overlay is hidden; when switching to teleprompter, the overlay is restored to its intended visibility state.
- **Telegram delivery**: Broadcast the detailed response to all active subscribers when `OUTPUT_METHOD=telegram`. When `TELEGRAM_BOT_TOKEN` is not set, a noop broadcaster is used. Messages exceeding Telegram's 4096-character limit are automatically split into sequential chunks. Empty AI responses are silently skipped. The HTTP client enforces a 30-second timeout per request; a non-responsive Telegram API will not stall the pipeline indefinitely.
- **Subscriber management**: Support dynamic subscriber registration via Telegram `/start` and `/stop` bot commands. Persist the subscriber list to a plain-text file (`TELEGRAM_SUBSCRIBER_STORE_PATH`, one chat ID per line) for durability across restarts. The poller idles when telegram is not the active output method; subscriber commands are ignored until `OUTPUT_METHOD=telegram`.
