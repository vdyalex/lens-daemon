...
|---|---|---|
| `TEST_POLL_INTERVAL` | `2s` | How often to check for screen changes (Go duration) |
| `TEST_DIFF_THRESHOLD` | `0.01` | Fraction of pixels that must differ to trigger processing (0.0–1.0) |
| `TEST_MAX_HISTORY` | `50` | Number of screenshots kept in the ring buffer |
| `TEST_SCREENSHOT_QUALITY` | `80` | JPEG quality for in-memory encoding (1–100) |
...
The daemon polls the screen every `TEST_POLL_INTERVAL` (default 2s). When the screen content changes beyond the diff threshold, it extracts text, sends it to Claude, and forwards the response to Telegram.
...
macOS will prompt for **Screen Recording** permission the first time test captures the screen. Grant access in **System Settings → Privacy & Security → Screen Recording**.
